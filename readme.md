...
Библиотека устанавливается в проект NODE JS средствами пакетного менеджера npm. 
Инсталляционный пакет необходимо скачать и распаковать во временную папку. Для установки нужно выполнить следующую команду:
...
Для обращения к библиотеке EDM следует в текст программы включить код:
...
2.	Клиент
Для установки Библиотека EDM на клиент в клиентский скрипт необходимо добавить следующую строку:

```html      
<script src='mypath/edm' ></script>
```

Здесь mypath/edm это ссылка на динамический ресурс, который должен возвращать собственно библиотеку, классы и конфигурации, созданные по модели данных. Ниже приведен код обработчика запросов, который возвращает Библиотеку EDM полностью подготовленную для работы на клиенте:
